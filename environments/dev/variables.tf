...
# Note: use_nat_gateway is NOT a variable here - it's hardcoded to false in main.tf
...
# AMI pinning (optional for dev)
variable "bastion_ami_id" {
  description = "AMI ID for bastion. If null, uses latest Ubuntu 24.04 LTS."
  type        = string
  default     = null
...
# Skip apt-get upgrade (disabled by default for dev - allows updates)
variable "skip_apt_upgrade" {
  description = "Skip apt-get upgrade in user_data."
  type        = bool
  default     = false
}

# Security group egress restrictions (open by default for dev)
variable "egress_allowed_cidrs" {
  description = "CIDR blocks allowed for egress (HTTP/HTTPS/DNS/ICMP). Default allows all for development."
  type        = list(string)
  default     = ["0.0.0.0/0"]
}
...
variable "flow_logs_kms_key_arn" {
  description = "KMS key ARN for encrypting VPC flow logs. Optional for dev, recommended for production."
  type        = string
  default     = null
}
